...
## vuex
  ...
  - state    对象
  - mutations  对象  
    -  mutation 函数  只能接收两个参数 第一个 state， 第二个参数载荷数据(修改 state 需要的数据)

  组件内使用
  - main.js 中挂载 store
  - 组件中使用 $store.state 获取   (一般写成计算属性)
  - 修改 state 的话 ，使用 $store.commit 调用对应的 mutation 函数, 也可以借助 mapMutations 辅助函数来取 mutation 并执行
